...
function TiledMap_DrawNearCam (camx,camy)
    ...
    for y = miny,maxy do
        ...
        if (gfx) then
			-- I don't know why these offsets are needed here. It should be fixed.
            local sx = (x-1)*kTileSize - camx + screen_w/2
            local sy = (y-1)*kTileSize - camy + screen_h/2
...
local function getLayers(node)
    local layers = {}
    for k, sub in ipairs(node) do
        if (sub.label == "layer") and sub[1].label == "data" then --  and sub.xarg.name == layer_name
            local layer = {}
...
local function getWorldObjects(node)
	local objects = {}
	for k, sub in ipairs(node) do
...
local function getMapData(node)
	...
	if (node.label == "map") then
       	d.width = node.xarg.width
		d.height = node.xarg.height
    end
	return d
end
